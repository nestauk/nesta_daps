...
class TestGeocoding:
    ...
    @mock.patch(REQUESTS)
    def test_request_includes_user_agent_in_header(
        self, mocked_request, mocked_osm_response
    ):
        mocked_request.return_value = mocked_osm_response
        geocode(something="a")
        assert mocked_request.call_args[1]["headers"] == {
            "User-Agent": "Nesta health data geocode"
        }

    @mock.patch(REQUESTS)
    def test_url_correct_with_city_and_country(
        self, mocked_request, mocked_osm_response
    ):
        mocked_request.return_value = mocked_osm_response
    ...
    @mock.patch(REQUESTS)
    def test_coordinates_extracted_from_json_with_one_result(
        self, mocked_request, mocked_osm_response
    ):
        mocked_request.return_value = mocked_osm_response
    ...
    def test_geocode_wrapper_rejects_both_q_and_kwargs_supplied(self, mocked_geocode):
        ...
        assert (
            "Supply either q OR other query parameters, they cannot be combined."
            in str(e.value)
        )

    @mock.patch(GEOCODE)
    def test_geocode_wrapper_errors_if_no_query_parameters_supplied(
        self, mocked_geocode
    ):
        with pytest.raises(ValueError) as e:
            _geocode()
...
class TestGeocodeDataFrame:
    ...
    def test_underlying_geocoding_function_called_with_city_country(
        ...
        assert geocoded_dataframe.to_dict(
            orient="records"
        ) == expected_dataframe.to_dict(orient="records")
        assert mocked_geocode.mock_calls == expected_calls
...
class TestCountryIsoCode:
    ...
    def test_title_case_is_applied(self, mocked_pycountry):
        expected_calls = []
        names = ["united kingdom", "UNITED KINGDOM", "United kingdom"]
        mocked_pycountry.side_effect = [
            KeyError(),
            KeyError(),
            KeyError(),
            "blah",
...
class TestCountryIsoCodeDataframe:
    ...
    def test_valid_countries_coded(self, mocked_country_iso_code):
        ...
        mocked_country_iso_code.side_effect = [
            mocked_response_uk,
            mocked_response_be,
            mocked_response_us,
        ]
...
def test_generate_composite_key():
    ...
    assert (
        generate_composite_key("Name-with hyphen", "COUNTRY")
        == "name-with-hyphen_country"
    )
...
def test_country_continent_lookup():
    ...
    non_nulls = {k: v for k, v in lookup.items() if k is not None and k != ""}
